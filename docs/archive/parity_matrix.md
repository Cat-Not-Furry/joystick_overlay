...
windows_ref: e924195 (2026-05-18, Fase 2)
```
...
|---------|-------|---------|--------|------|---------|-----------------|-------------------|------------|------------|-------|
| Update ZIP (runtime) | OK (mecánica PAR-005A) | PARCIAL (cli seguro; install `extractall`) | Transicional | Transitional | Pendiente | No | installer | PAR-005A, PAR-005B | SEC-001 | L: `safe_zip_update_extract`; W: `cli.py` + `safe_zip_extract`; `install/windows/install_ops.py` L56–57 |
...
| Reset dos fases | implementado | implementado (VM pendiente) | PARCIAL | Canonical | Pendiente | No | — | PAR-003 | SEC-003 | L: `fcntl` lock migración; W: lock no atómico |
...
| CI / QA automatizado | PARCIAL (ci.yml) | ausente | PARCIAL | Canonical | Pendiente | No | — | — | OPS-001 | L: pytest+links fail; W: sin workflows |
| ZIP perfil (import) | OK | OK | Igual | Canonical | Exacta | No | — | — | — | W: `profile_export.py` → `extract_zip_safely` |
| Monolitos / deuda LOC | PARCIAL | PARCIAL | PARCIAL | Canonical | Pendiente | Sí | — | — | ARCH-001, ARCH-002 | L: `main.py` ~140 LOC post ARCH-001; W: `main.py` 1265 LOC, `run_hud_layout_editor` ~409 LOC |
| Canal release usuario | PARCIAL | PARCIAL | PARCIAL | Canonical | Pendiente | No | — | PAR-005B | OPS-002 | L: `CHANGELOG.md` 0.3.2; W: sin CHANGELOG |
...
- **SEC-001** — Pipeline ZIP/update semánticamente incoherente (W: `install_ops.extractall`; L mitigado en update).
- **SEC-002** — Concurrencia `input_state` (L mitigado; W N/E backend distinto).
- **SEC-003** — Lock de migración (L mitigado `fcntl`; W patrón `isfile`+`open`).
- **REL-001** — Drift versión runtime / metadatos release (W confirmado).
