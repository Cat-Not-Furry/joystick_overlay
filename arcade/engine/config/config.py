...
def _external_data_root():
	"""Raíz de datos de aplicación fuera del repo (espejo de perfiles, XDG/AppData)."""
	if os.name == "nt":
		local = os.environ.get("LOCALAPPDATA")
		if local:
			return Path(local) / "joystick_overlay"
		return Path.home() / "AppData" / "Local" / "joystick_overlay"
	return Path.home() / ".local" / "share" / "joystick_overlay"


# Canon operativo: todo estado editable bajo el proyecto (PAR-001).
PROJECT_USER_DIR = str(PROJECT_ROOT / "user")
USER_DIR = PROJECT_USER_DIR
...
# Ventana
WINDOW_CAPTION_APP = "Joystick Overlay"
SCREEN_WIDTH = 375
SCREEN_HEIGHT = 175
