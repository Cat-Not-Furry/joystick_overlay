# profiles_io.py — crear, renombrar, exportar e importar perfiles

from app.ui.modals import _run_choice_menu, _run_message_modal, _run_text_input
...
def handle_create_profile(menu, active_profile, window_mode):
	create_profile(menu.profile_data, get_active_profile(menu.profile_data))
	return None


def handle_rename_profile(menu, active_profile, window_mode):
	t = _run_text_input(
		menu.screen,
...
def handle_export_profile(menu, active_profile, window_mode):
...
def handle_import_profile(menu, active_profile, window_mode):
