...
def _get_button_positions_base(button_count):
	if button_count == 4:
		return [
			...
			(315, 50),   # HP - columna der, arriba
			(315, 130),  # HK - columna der, abajo
		]
	if button_count == 8:
		full_positions = {
			"LP": (165, 50), "MP": (225, 50), "HP": (285, 50), "TR": (345, 50),
			"LK": (165, 130), "MK": (225, 130), "HK": (285, 130), "BR": (345, 130),
		}
	else:
		full_positions = {
...
def _get_button_positions_hitbox_mixbox_base(button_count):
	...
	if button_count == 4:
		return {
			"LP": (95 + dx, 130), "LK": (165 + dx, 50), "HP": (235 + dx, 50), "HK": (305 + dx, 50),
		}
	if button_count == 8:
		return {
			"LP": (95 + dx_8, 90), "MP": (165 + dx_8, 50), "HP": (235 + dx_8, 50), "TR": (345 + dx_8, 50),
			"LK": (95 + dx_8, 130), "MK": (165 + dx_8, 90), "HK": (235 + dx_8, 90), "BR": (345 + dx_8, 90),
		}
	# 6 botones
	return {
		"LP": (95 + dx, 90), "MP": (195 + dx, 50), "HP": (295 + dx, 50),
		"LK": (95 + dx, 130), "MK": (195 + dx, 90), "HK": (295 + dx, 90),
	}
...
def get_controller_button_name(label, controller_style):
	controller_maps = {
		"playstation": {"LP": "Cuadrado", "MP": "Triangulo", "HP": "R1", "TR": "R2", "LK": "X", "MK": "Circulo", "HK": "L1", "BR": "L2"},
		"xbox": {"LP": "X", "MP": "Y", "HP": "RB", "TR": "RT", "LK": "A", "MK": "B", "HK": "LB", "BR": "LT"},
		"switch": {"LP": "Y", "MP": "X", "HP": "R", "TR": "ZR", "LK": "B", "MK": "A", "HK": "L", "BR": "ZL"},
	}
...
def get_hud_fallback_text(label, controller_style):
	playstation_label_short = {"LP": "SQ", "MP": "TRI", "HP": "R1", "TR": "R2", "LK": "X", "MK": "O", "HK": "L1", "BR": "L2"}
	short_maps = {
		"playstation": {"Cuadrado": "SQ", "Triangulo": "TRI", "Circulo": "O", "X": "X"},
